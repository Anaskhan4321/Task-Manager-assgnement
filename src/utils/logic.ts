...
function normalizeTime(time: number): number | null {
...
export function withDerived(task: Task): DerivedTask {
  const normalizedTime =
    Number.isFinite(task.timeTaken) && task.timeTaken > 0
      ? task.timeTaken
      : null;

  const roi = computeROI(task.revenue, normalizedTime ?? 0);

  const displayRevenue =
    roi === null || normalizedTime === null
      ? null
      : task.revenue;

  return {
    ...task,
    timeTaken: normalizedTime ?? 0,
    roi,
    displayRevenue, 
...
export function computeTotalRevenue(tasks: ReadonlyArray<Task>): number {
  return tasks
    ...
    .reduce((sum, t) => {
      const revenueValid =
        Number.isFinite(t.revenue) && t.revenue > 0;
      const timeValid =
        Number.isFinite(t.timeTaken) && t.timeTaken > 0;
      const roiValid =
        computeROI(t.revenue, t.timeTaken) !== null;

      if (revenueValid && timeValid && roiValid) {
        return sum + t.revenue;
      }
...
export function computeTotalTimeTaken(tasks: ReadonlyArray<Task>): number {
  return tasks.reduce((sum, t) => {
    return Number.isFinite(t.timeTaken) && t.timeTaken > 0
      ? sum + t.timeTaken
      : sum;
  }, 0);
}
...
export function computeRevenuePerHour(tasks: ReadonlyArray<Task>): number {
  ...
  if (!Number.isFinite(revenue) || !Number.isFinite(time) || time <= 0) {
    return 0;
  }
...
export function computeAverageROI(tasks: ReadonlyArray<Task>): number {
  ...
  return Number(
    (rois.reduce((s, r) => s + r, 0) / rois.length).toFixed(2)
  );
}
