...
export function useTasks(): UseTasksState {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        console.log(res, 'No duplicate should be appear')
        ...
        if (isMounted) setTasks(dedupeById(finalData));
      } catch (e: any) {
  ...
  const addTask = useCallback((task: Omit<Task, 'id'> & { id?: string }) => {
    setTasks(prev => {
      ...
      return dedupeById([...prev, { ...task, id, timeTaken, createdAt, completedAt }]);
    });
  }, []);
  ...
  const undoDelete = useCallback(() => {
    ...
    setTasks(prev => {
      if (prev.some(t => t.id === lastDeleted.task.id)) {
        return prev;
      }
      const next = [...prev];
      ...
      next.splice(safeIndex, 0, lastDeleted.task);
      return next;
    });
  ...
  return {
    tasks, loading, error, derivedSorted, metrics, lastDeleted, addTask, updateTask, deleteTask, undoDelete, clearLastDeleted
  };
}
